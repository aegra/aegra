...
Pass `durability` (or the deprecated `checkpoint_during`) to control when every fired run writes checkpoints. For example, `durability="exit"` writes one checkpoint per run. See [checkpoint durability](/guides/threads-and-state#checkpoint-durability).
...
Supported update fields: `schedule`, `enabled`, `input`, `config`, `metadata`, `timezone`, `durability`.
