...
A separate `OrphanThreadSweeper` background task guards against a related leak: the stateless run endpoints (`POST /runs`, `/runs/wait`, `/runs/stream`) create an ephemeral thread and normally delete it once the run finishes, but a slow-client or dead-proxy disconnect can race ahead of the broker's end-event drain and skip that delete to avoid cancelling a still-active run. The sweeper periodically deletes ephemeral threads that are idle past `ORPHAN_THREAD_RETENTION_MINUTES` with no active run, so a missed fast-path delete doesn't leak the thread (and its checkpoints) forever. Unlike the opt-in [Thread TTL](/guides/threads-and-state#thread-ttl) sweeper, it runs unconditionally — it only ever touches threads the stateless endpoints marked ephemeral, never threads created via `POST /threads`.
...
| `services/orphan_thread_sweeper.py` | Background task deleting leaked ephemeral threads |
