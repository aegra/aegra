...
Aegra uses a JSON configuration file (`aegra.json`) to define graphs, authentication, HTTP settings, the semantic store, and checkpointer settings (thread TTL and the default checkpoint durability).
...
| `enable_custom_route_auth` | `bool` | `false` | Require Aegra auth on every FastAPI route in the custom app. See [custom routes](/guides/custom-routes#authentication-on-custom-routes) for what it covers |
...
Configure thread retention (TTL) and the default checkpoint durability.

### `ttl`

When a `ttl` block is present, every newly created thread gets an expiry, and
a background sweep deletes or compacts expired threads.
...
Sets when checkpoints are written for runs that do not choose a mode themselves.
...
| Field | Type | Default | Description |
|-------|------|---------|-------------|
| `durability` | `string` | unset | `sync`, `async`, or `exit`. `AEGRA_CHECKPOINT_DURABILITY` overrides it. With neither set, runs keep LangGraph's default, `async`. |

Resolution order for each run:

1. The run's `durability` field (or the deprecated `checkpoint_during`).
2. The `AEGRA_CHECKPOINT_DURABILITY` environment variable.
3. `checkpointer.durability` in `aegra.json`.
4. LangGraph's default.

An invalid value stops the server at startup. LangGraph Platform has no server-wide durability setting, so this key is specific to Aegra. See [checkpoint durability](/guides/threads-and-state#checkpoint-durability) for what each mode trades off.
