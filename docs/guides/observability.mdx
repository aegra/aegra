...
| | `outcome="stuck_pending"` | Pending runs missing from the job queue; re-enqueued without charging retry budget. Runs still waiting in the queue are not counted |
