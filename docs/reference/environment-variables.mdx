...
| Variable | Default | Description |
|----------|---------|-------------|
| `AEGRA_CHECKPOINT_DURABILITY` | — | Default checkpoint durability for runs that do not set one: `sync`, `async`, or `exit`. Wins over `checkpointer.durability` in `aegra.json`; a run's own `durability` wins over both. When unset, `checkpointer.durability` applies, and LangGraph's default (`async`) only if that is unset too |

```bash
# Write one checkpoint per run instead of one per step
AEGRA_CHECKPOINT_DURABILITY=exit
```

An invalid value stops the server at startup. See [checkpoint durability](/guides/threads-and-state#checkpoint-durability) for the trade-offs.
