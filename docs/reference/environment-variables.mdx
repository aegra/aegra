...
| Variable | Default | Description |
|----------|---------|-------------|
| `AEGRA_CHECKPOINT_DURABILITY` | — | Default checkpoint durability for runs that do not set one: `sync`, `async`, or `exit`. Wins over `checkpointer.durability` in `aegra.json`; a run's own `durability` wins over both. Unset keeps LangGraph's default (`async`) |
