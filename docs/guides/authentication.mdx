...
[Factory graphs](/reference/configuration#factory-graphs) receive a `ServerRuntime`
...
If you use `user.identity` to build a [store](/guides/store) namespace, qualify it with `"users"` — the API scopes every request, so an unqualified namespace is unreachable through `client.store.*`. See [Deep Agents and StoreBackend](/guides/store#deep-agents-and-storebackend).
