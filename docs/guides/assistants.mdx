...
### Custom LLM (model / base_url / api_key)

The example agent reads `model` / `base_url` from the assistant's `context`; credentials go in `secrets` (encrypted at rest, never returned). Unset fields fall back to the `OPENAI_MODEL` / `OPENAI_BASE_URL` / `OPENAI_API_KEY` environment variables:
...
<Note>
  Put credentials in `secrets` (not `config`/`context`): values are Fernet-encrypted at rest, never returned in any response, and decrypted into the runtime context only during execution. Requires `AEGRA_SECRET_KEY` to be set. Non-secret fields like `model` / `base_url` stay in `context`.
</Note>
...
# Search by user_id / tenant_id (narrows only within the caller's authorization boundary)
results = await client.assistants.search(user_id="alice", tenant_id="acme")
```
