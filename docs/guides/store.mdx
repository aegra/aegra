...
So a `StoreBackend` namespace **must be fully qualified**. A graph writes to exactly the tuple it asks for, so with no `"users"` head the write lands on an unscoped prefix: an agent writing under `("<identity>", "filesystem")` stores at `<identity>.filesystem`, while a read through the API for that same namespace is rewritten to `("users", "<identity>", "<identity>", "filesystem")` and finds nothing. The agent reads its own memories back while the REST API sees an empty store. With auth disabled the identity is `anonymous`: the write lands on `anonymous.filesystem` and the API looks under `users.anonymous.anonymous.filesystem`.

Separately, the identity itself has to come from somewhere Aegra populates. The Deep Agents docs suggest `rt.server_info.user.identity`, but `server_info` is metadata injected by LangGraph Server, which Aegra doesn't run, so it is `None` here and that expression fails before any write. Resolve the identity in a [factory graph](/reference/configuration#factory-graphs) instead — `runtime.user` there is the full `User` model, including custom fields from your auth handler. It can also be `None`, because Aegra calls the factory for introspection outside a request, so guard it:
...
async def graph(runtime: ServerRuntime):
    """Factory graph — runtime.user is None during Aegra's introspection calls."""
    user = runtime.user
    # Introspection passes no user; "anonymous" is the identity Aegra itself
    # uses when auth is disabled, so those rows stay reachable over REST.
        ...
        backend=CompositeBackend(
            ...
            routes={
                ...
                "/memories/": StoreBackend(
                    namespace=lambda rt: ("users", identity, "filesystem")
                )
            },
        ),
    )
...
Because the namespace now starts with `["users", <your identity>]`, the API passes it through unchanged, so read, list, and edit all address the rows the agent wrote. Keys are the file paths inside the routed directory, and values use the backend's file shape — `content` plus an `encoding`:

```python
# The "/memories/" route prefix is stripped, so the key is "/pref.txt".
await client.store.search_items(["users", "alice", "filesystem"])
await client.store.list_namespaces(["users", "alice"])
await client.store.get_item(["users", "alice", "filesystem"], "/pref.txt")
await client.store.put_item(
    ["users", "alice", "filesystem"],
    "/pref.txt",
    {"content": "Answer concisely.", "encoding": "utf-8"},
)
```

<Note>
  With a [configured scope](#configurable-scoping), qualify with that scope's prefix and your own attribute values instead — `("orgs", <your org_id>, "filesystem")` for a `"orgs": ["org_id"]` scope. Scopes are opt-in, so an unqualified namespace never resolves to one: it always falls back to per-user scope.
</Note>

Qualifying the namespace doesn't weaken the [isolation](#namespace-scoping) described above: Bob's requests are still buried under `["users", "bob", ...]` even when he asks for Alice's namespace, so he never sees her items.
