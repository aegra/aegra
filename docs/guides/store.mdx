...
So a `StoreBackend` namespace **must be fully qualified**. The Deep Agents docs suggest `(rt.server_info.user.identity, "filesystem")`; on Aegra that writes to the physical prefix `<identity>.filesystem`, while a read through the API for `("<identity>", "filesystem")` is rewritten to `("users", "<identity>", "<identity>", "filesystem")` and returns nothing. The agent reads its own memories back, and the REST API sees an empty store. With auth disabled the identity is `anonymous`, so the graph writes to `anonymous.filesystem` and the API looks under `users.anonymous.anonymous.filesystem`.

Prefix the namespace with `"users"` and both sides line up:
...
Because the namespace now starts with the caller's own identity, the API passes it through unchanged, so read, list, and edit all address the rows the agent wrote:

```python
await client.store.search_items(namespace_prefix=["users", "alice", "filesystem"])
await client.store.list_namespaces(prefix=["users", "alice"])
await client.store.get_item(namespace=["users", "alice", "filesystem"], key="pref.txt")
await client.store.put_item(
...
<Note>
  The `namespace` lambda only receives the runtime, not the run config. If `rt.server_info` isn't available in your setup, resolve the identity once in a [factory graph](/reference/configuration#graph-factories) and capture it — `runtime.user` there is the full `User` model, including custom fields from your auth handler:
  ...
  async def graph(runtime: ServerRuntime):
      identity = runtime.user.identity
  ...
  See [Accessing user data in your graph](/guides/authentication#accessing-user-data-in-your-graph).
</Note>
