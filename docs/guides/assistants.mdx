...
An assistant is unique per owner in two ways: its `assistant_id`, and its
`graph_id` plus `config` pair. Either match counts as "already exists", so a
second create with the same graph and config returns the first assistant rather
than a duplicate. Creation is atomic, so callers racing on the same
