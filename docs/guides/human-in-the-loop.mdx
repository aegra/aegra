...
- **A paused thread refuses fresh input.** Creating a run with plain `input` on a
  ...
  until a resume clears it. Set `MULTITASK_PAUSED_THREAD_POLICY=admit` to admit fresh
  input instead (LangGraph Platform's behaviour): the run starts from `__start__` and
  the pending interrupt is discarded.
