...
示例 agent(`react_agent` 及 `aegra init` 生成的项目)从这些变量读取默认模型配置;每个 assistant 也可通过 `context`(`model` / `base_url` / `api_key`)按需覆盖,未设置时回退到这里的环境变量。

| Variable | Description |
|----------|-------------|
| `OPENAI_API_KEY` | OpenAI API key |
| `OPENAI_MODEL` | 默认模型,`provider/model` 形式(如 `openai/gpt-4o-mini`) |
| `OPENAI_BASE_URL` | OpenAI 兼容端点的 base URL(自建代理或兼容服务) |
