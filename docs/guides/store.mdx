...
So a `StoreBackend` namespace **must be fully qualified**. The Deep Agents docs suggest `(rt.server_info.user.identity, "filesystem")`, which on Aegra writes to the physical prefix `<identity>.filesystem` — a read through the API for `("<identity>", "filesystem")` is rewritten to `("users", "<identity>", "<identity>", "filesystem")` and finds nothing, so the agent reads its own memories back while the REST API sees an empty store. With auth disabled the identity is `anonymous`: the graph writes to `anonymous.filesystem` and the API looks under `users.anonymous.anonymous.filesystem`.

`rt.server_info` is metadata injected by LangGraph Server, which Aegra doesn't run, so it is `None` here. Resolve the identity yourself in a [factory graph](/reference/configuration#factory-graphs) and capture it — `runtime.user` there is the full `User` model, including custom fields from your auth handler:
...
async def graph(runtime: ServerRuntime):
    """Factory graph — runtime.user is the authenticated user."""
...
Because the namespace now starts with `["users", <your identity>]`, the API passes it through unchanged, so read, list, and edit all address the rows the agent wrote:
