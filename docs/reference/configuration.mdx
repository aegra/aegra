...
def graph(config: dict):
    # Absent outside run execution, so read it with a default rather than by key.
...
```python
def graph(runtime: ServerRuntime):
    assistant_id = runtime.assistant_id  # type: ignore[union-attr]
    ...
```

`ServerRuntime` is the SDK's own union and does not declare `assistant_id`;
Aegra adds it on the runtime it passes. A type checker therefore rejects the
attribute, and there is no exported Aegra runtime type to annotate instead yet.
Read it from `config` where that matters, or suppress the one line.
