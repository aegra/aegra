...
### 自定义 LLM(model / base_url / api_key)

示例 agent 从 assistant 的 `context` 读取 LLM 配置,可为不同 assistant 指向不同模型或 OpenAI 兼容端点;未设置的字段回退到 `OPENAI_MODEL` / `OPENAI_BASE_URL` / `OPENAI_API_KEY` 环境变量:
...
<Note>
  存入 `context`(或 `config`)的 `api_key` 会持久化,但在 `get` / `search` 响应中会被脱敏为 `***` —— 不影响 graph 执行读取真实值。敏感场景建议改用环境变量。
</Note>
...
# Search by user_id / tenant_id (只在调用者的授权边界内收窄)
