...
- **SQL-layer authorization.** Every tenant-scoped read/write needs `user_id == user.identity` in the WHERE, even with `@auth.on` registered (default-allow when no handler — see GHSA-m98r-6667-4wq7). Routes taking `thread_id`/`assistant_id`/`cron_id` path params verify ownership + 404 at handler entry, not deeper. **Do not hand-write the filter** — route queries through `core/authz.py`: `owner_filter(Model, user)` in API routes, `scope(Model, identity, tenant_id)` in services (which hold a string id, not a `User`), and `owns(row, user)` for post-fetch checks. These apply the optional `tenant_id` composite isolation (`tenant_id IS NOT DISTINCT FROM :t`, NULL-safe so a tenant-less caller matches NULL rows) on top of `user_id`. Adding a new resource means wiring its queries through these helpers so the tenant boundary can never be missed. Secrets in responses (`config`/`context` api_key etc.) are masked via `utils/redaction.py` `redact_secrets` at the serialization boundary — storage/execution keep the real value.
