...
A request without valid credentials then gets `401` on every FastAPI route your app declares, including routes from included routers and from mounted FastAPI sub-apps. Aegra's own routes are not affected: `/health`, `/live`, `/ready`, `/info`, `/` and the docs pages stay public, and the Agent Protocol routes keep their usual auth.

Three kinds of route cannot take a FastAPI dependency, so this setting does not cover them: plain Starlette routes added with `app.add_route`, websocket routes, and mounted non-FastAPI apps such as `StaticFiles`. Aegra logs a startup warning that lists any it finds. Protect those inside the route itself.
