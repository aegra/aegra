...
- **SQL-layer authorization.** Every tenant-scoped read/write needs `user_id == user.user_id` in the WHERE, even with `@auth.on` registered (default-allow when no handler — see GHSA-m98r-6667-4wq7). Routes taking `thread_id`/`assistant_id`/`cron_id` path params verify ownership + 404 at handler entry, not deeper. **Do not hand-write the filter** — route queries through `core/authz.py`: `owner_filter(Model, user)` in API routes, `scope(Model, user_id, tenant_id)` in services (which hold a string id, not a `User`), and `owns(row, user)` for post-fetch checks. These apply the optional `tenant_id` composite isolation (`tenant_id IS NOT DISTINCT FROM :t`, NULL-safe so a tenant-less caller matches NULL rows) on top of `user_id`. Adding a new resource means wiring its queries through these helpers so the tenant boundary can never be missed. Per-assistant secrets live in the `secrets` field, Fernet-encrypted at rest (`core/crypto.py`, keyed by `AEGRA_SECRET_KEY`), never serialized in responses, and decrypted into the runtime `context` only at execution (`run_executor._resolve_context`) — never into `config.configurable`, so they never reach checkpoints. `config`/`context` are not secret stores; `redact_secrets` now masks only cron webhook headers. Run identity keys (`thread_id`/`run_id`/`assistant_id`) are server-pinned into `config.configurable` at execution (`create_run_config` overwrite + `SERVER_PINNED_CONFIG_KEYS` for checkpoint params) — client overrides are ignored, so graph factories can trust `configurable["assistant_id"]` for per-assistant resources. Assistant sharing (`core/sharing.py` `visible_assistant_filter`) widens **read/execute** visibility to shared rows (user/tenant/public via `assistant_share`) while **write** paths stay `owner_filter`; grantees execute using the owner-stored keys by design. Broad cross-user / cross-tenant queries use `query_scope` (internal only — request endpoints stay composite-isolated).
