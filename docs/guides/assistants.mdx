...
An assistant has two uniqueness keys: its `assistant_id`, which is unique across
...
<Note>
  A client-provided `assistant_id` must be 1–255 non-blank characters. Omit the field (or send `null`) to let the server generate a UUID. Longer or blank values are rejected with 422 before they hit PostgreSQL's btree key limit (~2704 bytes).
</Note>
...
Updates are partial: a field you leave out keeps the value it already has. `config` and `context` replace, so sending either one explicitly empty clears it:
...
`metadata` is different: it merges into the assistant's existing metadata rather than replacing it, so `metadata={}` changes nothing and a stored key cannot be removed through this endpoint — only overwritten.

`name` and `graph_id` are stored as non-null, so supplying either one empty is a `422` rather than a silent no-op. Omit them to leave them unchanged. `description` is nullable, so an explicit `None` clears it.
...
Retrieve the schemas for an assistant's graph:

```python
schemas = await client.assistants.get_schemas(assistant_id)
print(schemas["graph_id"])
print(schemas["input_schema"])
print(schemas["output_schema"])
print(schemas["state_schema"])
print(schemas["config_schema"])
print(schemas["context_schema"])  # the context type a graph factory declares
```

Every key is always present. A schema the server could not derive from the graph is `null`, so read the value rather than testing for the key.
