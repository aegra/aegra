...
Building a store namespace from `user.identity`? Qualify it with `"users"` or it's unreachable over REST. See [Deep Agents and StoreBackend](/guides/store#deep-agents-and-storebackend).
