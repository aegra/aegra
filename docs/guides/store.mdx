...
Scoping applies only to the REST API; a graph writes whatever namespace it asks for. See [Deep Agents and StoreBackend](#deep-agents-and-storebackend).
...
A `StoreBackend` namespace must be fully qualified, because the REST API buries anything not already headed by `["users", <identity>]` — see [namespace scoping](#namespace-scoping).

An unqualified `("<identity>", "filesystem")` stores at the physical prefix `<identity>.filesystem`, while the same namespace over REST is rewritten to `("users", "<identity>", "<identity>", "filesystem")` and finds nothing. The agent reads its own memories; the API sees an empty store. With auth off the identity is `anonymous` — written to `anonymous.filesystem`, read from `users.anonymous.anonymous.filesystem`.

Deep Agents documents `rt.server_info.user.identity`. On Aegra `server_info` is `None`, so that path errors before any write. Use a [factory graph](/reference/configuration#factory-graphs) and `runtime.user` instead, which is `None` during introspection:
...
async def graph(runtime: ServerRuntime):
    """Factory graph."""
    user = runtime.user  # None during Aegra introspection
...
`("users", <identity>, ...)` passes through scoping unchanged, so `client.store.*` addresses the same rows. Keys are paths inside the routed directory — `/memories/` is stripped — and values use the backend's file shape:
...
Qualifying the namespace doesn't weaken the [isolation](#namespace-scoping) described above: Bob's requests are still buried under `["users", "bob", ...]`, so he never sees Alice's items.
