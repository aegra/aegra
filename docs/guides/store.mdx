...
A `StoreBackend` namespace must be fully qualified: the REST API buries anything not already headed by `["users", <identity>]` (see [namespace scoping](#namespace-scoping)).

An unqualified `("<identity>", "filesystem")` stores under `<identity>.filesystem`. Over REST that namespace is rewritten to `("users", "<identity>", "<identity>", "filesystem")` and finds nothing. The agent reads its own memories; the API sees an empty store. With auth off the identity is `anonymous` — written to `anonymous.filesystem`, read from `users.anonymous.anonymous.filesystem`.
...
Qualifying the namespace doesn't weaken the [isolation](#namespace-scoping): Bob's requests are buried under `["users", "bob", ...]`, so he never sees Alice's items.
