...
tick (a fire that outlasts the poll interval never completes), and with `reject` every
occurrence that lands while the prior fire is still active is skipped (logged, and
`next_run_date` advanced) rather than queued. Also note: if the bound thread is paused on
a human-in-the-loop `interrupt()`, cron occurrences are skipped the same way (regardless
of strategy, including `enqueue`) until the thread is resumed. See
