---
title: "Observability"
description: "Set up native LangSmith tracing or OpenTelemetry fan-out."
---

Aegra supports native LangSmith tracing for Agent Protocol clients and LangGraph Studio, alongside OpenTelemetry fan-out for other observability backends.

## Supported backends

Out of the box, Aegra supports:

- **LangSmith** — Native LangChain traces with Agent Protocol run-session lookup
...
Tracing is configured entirely through environment variables in your `.env` file. Native LangSmith tracing and OpenTelemetry tracing are independent and can be enabled together.

### Native LangSmith tracing

Enable native tracing when LangGraph Studio needs to open a run's trace:

```bash
LANGSMITH_TRACING=true
LANGSMITH_ENDPOINT=https://api.smith.langchain.com
LANGSMITH_API_KEY=lsv2_...
LANGSMITH_PROJECT=my-aegra-project
# Required for organization-scoped API keys
LANGSMITH_WORKSPACE_ID=...
```

Aegra records the resolved project in each Agent Protocol run response as `langsmith_session_name`. A run request can select an additional project with `langsmith_tracer.project_name` and associate a dataset example with `langsmith_tracer.example_id`, which must be a UUID string. If no per-run project is supplied, `LANGSMITH_PROJECT` is used.

Both fields are independent, matching the LangGraph SDK's `langsmith_tracing` argument: `example_id` alone associates the trace with a dataset example in the default project, without needing `project_name`. Unknown fields are ignored rather than rejected, so a newer SDK can talk to an older Aegra.

Set `LANGSMITH_TRACING=false` to disable native trace export. The run response then contains a null `langsmith_session_name`.

<Note>
`/info` advertises `flags.langsmith_tracing_session_on_runs` so Studio knows the server returns `langsmith_session_name`. The flag only states the capability — that Studio then opens the session is a manual acceptance step against a real deployment. To prove traces actually reach LangSmith, run the opt-in export test with a real key: `LANGSMITH_API_KEY=lsv2_... uv run --package aegra-api pytest libs/aegra-api/tests/e2e/test_runs/test_langsmith_export_live.py`.
</Note>
...
The OpenTelemetry path works as follows:
...
|----------|-------------|
| `LANGSMITH_TRACING` | Enable native LangSmith traces and run-session recording (`true`/`false`) |
| `LANGSMITH_ENDPOINT` | LangSmith API endpoint |
| `LANGSMITH_API_KEY` | LangSmith API key |
| `LANGSMITH_PROJECT` | Default LangSmith project and run session name |
| `LANGSMITH_WORKSPACE_ID` | Workspace ID required by organization-scoped API keys |
...
In production mode (`REDIS_BROKER_ENABLED=true`), the [lease reaper](/guides/worker-architecture) reports the runs it recovers:

| Metric | Labels | Meaning |
|--------|--------|---------|
| `aegra_reaper_recovered_runs_total` | `outcome="crashed_retried"` | Runs whose worker lease expired; reset to pending and re-enqueued |
| | `outcome="crashed_exhausted"` | Crashed runs that exceeded `BG_JOB_MAX_RETRIES`; marked permanently failed |
| | `outcome="stuck_pending"` | Runs never claimed by a worker; re-enqueued without charging retry budget |

The counter increments only for confirmed operations: a successful Redis push for `crashed_retried`/`stuck_pending`, a successful DB update for `crashed_exhausted`. If Redis is down during recovery, workers still pick the runs up via the Postgres poll fallback, but those recoveries are not counted (the metric never overcounts, though it can undercount during a Redis outage).

All three series are exported as `0` from startup, so `rate()` queries and alerts work before the first recovery. A sustained non-zero rate of `crashed_retried` means workers are dying mid-run (check memory limits and crash loops); any growth in `crashed_exhausted` means runs are being lost permanently and is worth alerting on. In dev mode (`aegra dev`) the reaper does not run and these counters stay at zero.
