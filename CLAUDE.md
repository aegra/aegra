...
- **One exception: fields that configure a system outside Aegra.** A field whose only effect is on a LangSmith-side service (for example `langsmith_tracer`, `feedback_keys`) may be accepted and left inert, because no value of it changes what Aegra does. A field that is inert only for some values is not covered: accept the values that already match current behavior and 422 the rest. Each inert field is listed in `docs/feature-support.mdx` with the reason. Nothing else may be inert.
...
- **Pin the contract with a drift test, on behavior not names.** v2: `tests/unit/test_services/test_event_streaming/test_spec_params.py` vendors the protocol param shapes, asserts the handler key sets cover them, and asserts each key reaches the run or is a listed no-op. A key-set check alone passes while a declared field is silently dropped, so assert the value lands and assert the 422 for values that are not supported. v1: the request-model drift test is tracked in #503; until it lands, add both assertions with each new field. A new SDK field must fail CI, not reach a user.
