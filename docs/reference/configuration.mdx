...
```json
{
  ...
  "http": {
    ...
    "cors": {
      ...
      "allow_origin_regex": "^https://[a-z0-9-]+\\.example\\.com$",
...
| `cors.allow_origins` | `list[str]` | `["*"]` (`[]` if `allow_origin_regex` is set) | Allowed CORS origins |
| `cors.allow_origin_regex` | `string` | `None` | Regular expression for additional allowed origins; when present without `allow_origins`, the origin-list default is `[]` |
| `cors.allow_credentials` | `bool` | `false` with a regex or wildcard origins, `true` with concrete origins and no regex | Allow credentials in CORS requests |

When `allow_origin_regex` is non-null, it is passed to Starlette's `CORSMiddleware` and matched against the complete origin. If `allow_origins` is also provided, either the list or regex can authorize an origin. An explicit `allow_origins: ["*"]` remains a wildcard. An empty string still counts as configured; it does not match normal non-empty origins.
